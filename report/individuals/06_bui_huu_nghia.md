# Member Role Report — Bùi Hữu Nghĩa
...
| Vai trò chính | Data observability, corruption/repair evidence và reporting |
...
|---|---|---|---|---|
| Quality checks, freshness, corruption scenarios, comparison report và tests | src/observability/quality.py; src/observability/reporting.py; src/ingestion/corruption.py | Clean dataframe, frozen test set, raw records và metrics ba state | quality/freshness reports, corruption log, comparison report và test coverage | Hoàn thành |
...
|---|---|---|
| Cung cấp evidence về data defect, repair và tác động downstream | Pipeline tích hợp của nhóm | Artifact và report có thể tái hiện; pytest đã pass |

## 3. Kết quả theo vai trò

| Nhiệm vụ đã thực hiện | File/hàm/artifact liên quan | Kết quả bàn giao | Cách xác minh |
|---|---|---|---|
| Triển khai phần việc sở hữu | src/observability/quality.py; src/observability/reporting.py; src/ingestion/corruption.py | quality/freshness reports, corruption log, comparison report và test coverage | uv run pytest -q |
| Tích hợp với pipeline | data/results và data/reports | Corrupted state có 198 rows/1 duplicate/Valid=False; repaired quay về 197 rows/0 duplicate/Valid=True. | Chạy phase1 và corruption flow |

Output cụ thể: Corrupted state có 198 rows/1 duplicate/Valid=False; repaired quay về 197 rows/0 duplicate/Valid=True.

## 4. Giải thích phần kỹ thuật đã thực hiện

### Vấn đề cần giải quyết

Phần việc của tôi phải đảm bảo quality checks, freshness, corruption scenarios, comparison report và tests hoạt động theo contract và tạo evidence có thể kiểm chứng cho các bước kế tiếp.

### Cách triển khai

Tạo blank summary, stale date, duplicate ID và embedding noise, bắt buộc overlap với frozen doc IDs. Report kiểm tra hash/sample count trước khi ghi comparison; repair được đánh giá từ raw -> cleaning -> reindex.

### Input, output và contract

| Thành phần | Mô tả |
|---|---|
| Input | Clean dataframe, frozen test set, raw records và metrics ba state |
| Output | quality/freshness reports, corruption log, comparison report và test coverage |
...
- **Kết quả thực tế:** Corrupted state có 198 rows/1 duplicate/Valid=False; repaired quay về 197 rows/0 duplicate/Valid=True.
...
- **Phương án đã chọn:** Chọn corruption có overlap với test set thay vì làm hỏng row ngẫu nhiên. Điều này bảo đảm thí nghiệm có cơ hội tạo downstream impact.
- **Lý do:** Ưu tiên correctness, reproducibility và khả năng audit hơn tối ưu ngắn hạn.
- **Bằng chứng quyết định phù hợp:** Corrupted state có 198 rows/1 duplicate/Valid=False; repaired quay về 197 rows/0 duplicate/Valid=True.

## 6. Một lỗi hoặc blocker đã xử lý

- **Triệu chứng/lỗi nguyên văn:** Corruption không làm hit rate giảm dù quality fail. Kiểm tra cho thấy target paper vẫn nằm top-k; do đó report không overclaim tác động retrieval.
...
|---|---:|---:|---:|---|
| retrieval_hit_rate | 1.0000 | 1.0000 | 1.0000 | F1 giảm 0.0127 sau corruption là signal có thật, còn quality recovery quan trọng hơn biến động wording của LLM. |
...
Thêm per-scenario impact, distribution drift và phân loại freshness chi tiết. Hiệu quả sẽ được đo bằng held-out metrics, per-scenario comparison và report có hash/config rõ ràng.

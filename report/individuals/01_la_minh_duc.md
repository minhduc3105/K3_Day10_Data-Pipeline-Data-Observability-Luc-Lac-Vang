# Member Role Report — Lã Minh Đức
...
| Vai trò chính | Nhóm trưởng, tích hợp pipeline và orchestration |
...
|---|---|---|---|---|
| Phase 1, corruption/repair flow, kiểm tra contract liên module | src/pipelines/phase1.py; src/pipelines/corruption_flow.py | raw records, clean dataframe, test set, index và metrics của các state | baseline/corrupted/repaired artifacts và các report cuối | Hoàn thành |
...
|---|---|---|
| Tích hợp các module ingestion, retrieval, evaluation và observability | Pipeline tích hợp của nhóm | Artifact và report có thể tái hiện; pytest đã pass |

## 3. Kết quả theo vai trò

| Nhiệm vụ đã thực hiện | File/hàm/artifact liên quan | Kết quả bàn giao | Cách xác minh |
|---|---|---|---|
| Triển khai phần việc sở hữu | src/pipelines/phase1.py; src/pipelines/corruption_flow.py | baseline/corrupted/repaired artifacts và các report cuối | uv run pytest -q |
| Tích hợp với pipeline | data/results và data/reports | Ba state dùng chung frozen hash, 10 samples; report chỉ ghi khi hash và sample count thống nhất. | Chạy phase1 và corruption flow |

Output cụ thể: Ba state dùng chung frozen hash, 10 samples; report chỉ ghi khi hash và sample count thống nhất.

## 4. Giải thích phần kỹ thuật đã thực hiện

### Vấn đề cần giải quyết

Phần việc của tôi phải đảm bảo phase 1, corruption/repair flow, kiểm tra contract liên module hoạt động theo contract và tạo evidence có thể kiểm chứng cho các bước kế tiếp.

### Cách triển khai

Điều phối thứ tự raw -> clean -> index -> evaluate -> quality. Repair luôn đọc lại raw records rồi chạy cleaning chuẩn, không vá corrupted dataframe.

### Input, output và contract

| Thành phần | Mô tả |
|---|---|
| Input | raw records, clean dataframe, test set, index và metrics của các state |
| Output | baseline/corrupted/repaired artifacts và các report cuối |
...
- **Kết quả thực tế:** Ba state dùng chung frozen hash, 10 samples; report chỉ ghi khi hash và sample count thống nhất.
...
- **Phương án đã chọn:** Chọn kiểm tra frozen hash trước khi so sánh thay vì chỉ ghép các file metrics. Điều này loại bỏ rủi ro kết luận từ hai test set khác nhau.
- **Lý do:** Ưu tiên correctness, reproducibility và khả năng audit hơn tối ưu ngắn hạn.
- **Bằng chứng quyết định phù hợp:** Ba state dùng chung frozen hash, 10 samples; report chỉ ghi khi hash và sample count thống nhất.

## 6. Một lỗi hoặc blocker đã xử lý

- **Triệu chứng/lỗi nguyên văn:** ChromaDB báo DuplicateIDError khi corruption nhân đôi paper_id. Record ID nội bộ được bổ sung row index trong khi paper_id nghiệp vụ giữ nguyên.
...
|---|---:|---:|---:|---|
| retrieval_hit_rate | 1.0000 | 1.0000 | 1.0000 | Tôi đánh giá repaired thành công trước hết ở lineage và quality; F1 chưa quay lại baseline không tự động chứng minh repair sai. |
...
Thêm run manifest chứa timestamp, config, model và hash mọi artifact cho từng lần chạy. Hiệu quả sẽ được đo bằng held-out metrics, per-scenario comparison và report có hash/config rõ ràng.
